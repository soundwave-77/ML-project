...
def preprocess_data(df_train, train_emb):
    print('==== Preparing Data ====')

    # dense_df_title = pd.DataFrame.sparse.from_spmatrix(train_emb)
    train_emb.columns = [f'title_embedding_{i}' for i in range(train_emb.shape[1])]

    df_train = pd.concat([df_train.reset_index(drop=True), train_emb], axis=1)
    ...
    num_features = ['price', 'image_top_1']
...
def train_model(model_name, task_name, X, y):
    ...
    for train_idx, val_idx in tqdm(FOLD_LIST):
        ...
        model.fit(X_train,
                  ...
                  eval_metric='rmse',
                  callbacks=[
                      early_stopping(stopping_rounds=5, verbose=True),
                      log_evaluation(50)])
...
def main(df_train, title_emb, model_name, task_name):
    # Preprocess data
    X, y = preprocess_data(df_train, title_emb)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--title_emb_path', type=str, required=True, help='Path to the title embeddings file')
    parser.add_argument('--model_name', type=str, default='LGBM', help='Name of the model to be trained')
    parser.add_argument('--task_name', type=str, default='lgbm_tfidf_title', help='Name of the ClearML task')

    args = parser.parse_args()

    train_path = args.train_path
    title_emb_path = args.title_emb_path
