...
def preprocess_data(df, stop_words, cat_features):
    ...
    df = df.drop(['image', 'item_id', 'user_id', 'activation_date', 'title', 'description'], axis=1, errors='ignore')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Make predictions using a trained CatBoost model.")
    parser.add_argument('--model-path', type=str, required=True, help='Path to the trained CatBoost model file.')
    parser.add_argument('--input-csv', type=str, required=True, help='Path to the input CSV file for prediction.')
    parser.add_argument('--output-csv', type=str, default='predictions.csv', help='Path to save the output predictions.')
